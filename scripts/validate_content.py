...
def main() -> None:
    ...
    validate_registry_integrity(image_catalog, image_registry, require_required_assets=False, require_current_files=False)
    emoji_catalog = load_emoji_catalog(validate_files=True)
    emoji_registry = load_emoji_registry()
    validate_emoji_asset_references(emoji_catalog)
    validate_emoji_registry_integrity(emoji_catalog, emoji_registry, require_current_files=False)
    ...
    print(
        ...
        f"{len(image_catalog.assets)} image assets, {len(image_registry.assets)} registered image assets, "
        f"{len(emoji_catalog.emojis)} emoji assets, {len(emoji_registry.emojis)} registered emoji assets, and progression content "
